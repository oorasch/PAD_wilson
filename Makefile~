...
##################################################################################################
PROGRAM	= main
SOURCES = main.cpp initialize.cpp determinant.cpp constants.cpp updates.cpp debug.cpp observables.cpp #rangen.cpp
...
#CXXFLAGS = -ffast-math -O3 -funroll-all-loops -DNDEBUG ${WARNING} -ftree-vectorizer-verbose=2 \
           # -fopenmp
CXXFLAGS = -std=c++11 -Ofast -march=native `gsl-config --cflags` -pedantic -Wall -Wextra
#CXXFLAGS = -std=c++11
# CXXFLAGS = -g
